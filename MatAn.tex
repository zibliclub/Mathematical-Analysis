...
\begin{proof}
  ...
  Пусть $f$ - неограничена, то есть $\forall M>0 \ \exists x\in[a;b]: \ | f(x) | > M$.\\

  Покажем, что в этом случае функция $\sigma_{f}: \Re\xi\rightarrow\mathbb{R}$ не имеет предела по базе
  $\lambda(P)\rightarrow0$. 
\end{proof}
