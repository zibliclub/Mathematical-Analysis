...
\begin{proof}
  ...
  \begin{enumerate}
    \item Пусть $\forall x \in [a;\omega) \ f(x)\leqslant g(x)$ и $\forall b \in [a;\omega) \ f,g \in R[a;b]$ сходится
          $\iff \forall b \in [a;\omega) \exists M > 0:$
          \begin{equation*}
            \int_{a}^{b}(g(x) - f(x))dx < M \implies
          \end{equation*}
          \begin{equation*}
            \int_{a}^{b}g(x)dx - \int_{a}^{b}f(x)dx < M \implies
          \end{equation*}
          \begin{equation*}
            \int_{a}^{b}g(x)dx < M + \int_{a}^{b}f(x)dx.
          \end{equation*}
          Перейдем к пределу при $b \rightarrow \omega$:
          \begin{equation*}
            \underset{b\rightarrow\omega}{\lim}\int_{a}^{b}g(x)dx < M + \underset{b\rightarrow\omega}{\lim}\int_{a}^{b}
            f(x)dx \ (\square).
          \end{equation*}
          Рассмотрим $G(b) = \int_{a}^{b}g(x)dx:$
          \begin{equation*}
            F(b) = \int_{a}^{b}f(x)dx
          \end{equation*}
    \item Если $\int_{a}^{\omega}f(x)dx$ расходится, то с учетом неравенства $(\square)$ расходится и
          $\int_{a}^{\omega} g(x)dx$.

          Теперь, пусть $\int_{a}^{\omega}g(x)dx$ сходится $\implies \exists \underset{b\rightarrow\omega}{\lim}
            G(b) \implies$
  \end{enumerate}
